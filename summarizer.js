...
Summarizer.Utility.makeGraph = function(sentences) {
  ...
  for(var idx1 = 0; idx1 < sentences.length; ++idx1) {
    for(var idx2 = idx1 + 1; idx2 < sentences.length; ++idx2) {
      ...
      graph[idx1].push({"node" : idx2, "weight" : similarityScore});
      graph[idx2].push({"node" : idx1, "weight" : similarityScore});
    }
  }
...
Summarizer.Utility.calculatePageRank = function(graph, maxIterations, dampingFactor, delta) {
  var pageRankStruct = {};
  var totalWeight = {};
  var totalNumNodes = graph.sentenceIdLookup.length; // Number of nodes.
  for(var idx = 0; idx < totalNumNodes; ++idx) {
    pageRankStruct[idx] = {"oldPR": 1.0, "newPR": 0.0};
    totalWeight[idx] = 0.0;
  }
  for(var idx = 0; idx < totalNumNodes; ++idx) {
  ...
  var pageRankResults = {};
  for(var idx = 0; idx < totalNumNodes; ++idx) {
    pageRankResults[idx] = {"PR": pageRankStruct[idx]["oldPR"] / totalNumNodes, "sentence": graph.sentenceIdLookup[idx]};
  }
